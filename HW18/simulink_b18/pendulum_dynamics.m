function [sys,x0,str,ts,simStateCompliance] = pendulum_dynamics(t,x,u,flag,P)
switch flag,
...
function sys=mdlDerivatives(t,x,u)
  ...
  if t==0,
    ...
    m1 = 0.25 * (1+2*alpha*rand-alpha);  % kg
    m2 = 1 * (1+2*alpha*rand-alpha);     % kg
    ell = 0.5 * (1+2*alpha*rand-alpha); % m
    b = 0.05 * (1+2*alpha*rand-alpha); % N m
    g = 9.8; % m/s^2
  end
